...
function randomParagraph(){
    ...
    // etting random items from the paragraphs array, splitting all characters of it, 
...
function initTyping(){
    ...
    if(timeLeft > 0 && charIndex < characters.length){
        if(!isTyping){ // once timer is start, it won't restart again on every key cliked
            timer = setInterval(initTimer, 1000)
            isTyping = true
        }
    
        // if user hasn't entered any character or pressed backspace button
        if(typedChar == null){
            ...
            if(characters[charIndex].innerText === typedChar){
                // if user tapped character andd shown character matched then add
        ...
        // if wpm is 0, or empty or inifinity then setting it's value to 0
